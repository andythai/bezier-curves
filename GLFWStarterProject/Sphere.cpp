...
void Sphere::solidSphere(GLuint shaderProgram, bool green) {
	...
	// Materials
	if (green == true) {
		glUniform3f(glGetUniformLocation(shaderProgram, "material.ambient"), 0.1f, 0.9f, 0.1f);
	}
	else {
...
// This function draws a solid sphere
void Sphere::selection_buffer(GLuint shaderProgram) {
	// No need to generate the sphere again if the previous rendering already
	// used the same number of stacks and slices
	glm::mat4 modelview = Window::V * toWorld;
	uProjection = glGetUniformLocation(shaderProgram, "projection");
	...
	idLocation = glGetUniformLocation(shaderProgram, "id");
